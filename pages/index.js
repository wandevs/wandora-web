...
class IndexPage extends Component {
    ...
    try {
      ...
      for (let i = 0; i < roundArray.length; i++) {
        ...
        trendHistory.push({
          key: roundArray[i],
          round: roundArray[i],
          startPrice: Number(ret.openPrice) / 1e8,
          endPrice: Number(ret.closePrice) / 1e8,
          result: (Number(ret.openPrice) > Number(ret.closePrice)) ? 'down' : (Number(ret.openPrice) < Number(ret.closePrice)) ? 'up' : 'draw',
          upAmount: Number(ret.upAmount) / 1e18,
          downAmount: Number(ret.downAmount) / 1e18,
  ...
  updateRandomHistoryFromNode = async () => {
    try {
      ...
      const address = selectedAccount ? selectedAccount.get('address') : "";
  ...
  getAmountInfo = () => {
    ...
    let upOdds = Number(this.state.trendInfo.upPoolAmount) === 0 ? "NaN" : (Number(this.state.trendInfo.downPoolAmount) / Number(this.state.trendInfo.upPoolAmount) * 0.99).toFixed(2);
    let downOdds = Number(this.state.trendInfo.downPoolAmount) === 0 ? "NaN" : (Number(this.state.trendInfo.upPoolAmount) / Number(this.state.trendInfo.downPoolAmount) * 0.99).toFixed(2);
  ...
  render() {
    return (
      <div className={style.app}>
        <Panel walletButton={WalletButtonLong} trendInfo={this.state.trendInfo} amountInfo={this.state.amountInfo} sendTransaction={this.sendTransaction} watchTransactionStatus={this.watchTransactionStatus} symbol={'WAN'} unit={'BTC / WAN'}/>
